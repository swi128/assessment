...
function assessment(userName){
  ...
  let sumOfCharCode = 0;
  for(let i = 0; i < userName.length/*文字列の長さを所得している*/; i++) {
    sumOfCharCode = sumOfCharCode + userName.charCodeAt(i);//charCodeAtは文字コードを所得する関数
  }

  //文字コードの番号の合計を回答の数で割って添字の数値を求める
  const index = sumOfCharCode % answers.length;
  let result = answers[index];//一番上のconst ansewrs =[...の数
